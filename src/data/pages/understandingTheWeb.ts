...
export default {
  ...
  content: Page(
    ...
    Body(
      ...
      Paragraph(
        Italic('One issue though… What’s a ', Bold('menu'), '?')
      ),
      Paragraph(
        'Well it’s ',
        Italic('obvious'),
      ...
      Paragraph('We’ve kept the ',
        Italic('nav'), ', ',
        Italic('header'), ' and ',
        Italic('footer'),
        ' from the HTML5 notation, and aggregated a bunch of the other tags under the ', Italic('content'), ' section. We’ve also included the form semantic tag and defined a new section control. Control is meant to define any menu bar or toolbar which contains a set of inputs used to sort or filter data.'),
      ...
      Paragraph('But nobody wants to go through thousands of websites and manually annotate each and every section… So we’ve built a chrome extension to partially automate the process by pre-parsing the website based on a collection of heuristics.'),
      Title2('Defining heuristics'),
      Paragraph(Italic('“A heuristic, is any approach to problem solving, learning, or discovery that employs a practical method, not guaranteed to be optimal, perfect, logical, or rational, but instead sufficient for reaching an immediate goal. ”'), ' — Wikipedia'),
      Paragraph('It was clear that we didn’t want to make all of this labeling work manually. We’ve started to dissect what makes us think that a certain element obviously serves a certain function. What makes a nav bar a nav bar? Well… it’s usually a list, it contains anchor tags, it’s either a long horizontal element in the top part of the page or a vertical one on the left, we might find certain keywords within its inner html and so on and so forth…'),
      ...
      Paragraph(
        Pre(
          'navScore = 0.5 * containsTags(&lt;UL&gt;, &lt;OL&gt;, &lt;LI&gt;)', Br,
          '        + 0.3 * containsTags(&lt;A&gt;)', Br,
      ...
      Paragraph(
        ...
        ExternalLink('Kernel Selection and Dimensionality Reduction in SVM Classification of Autism Spectrum Disorders', 'http://www.cs.utah.edu/~ssingla/Machine%20Learning/project.pdf'),
